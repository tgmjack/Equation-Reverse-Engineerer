...
##### update notes  0.42
# i think i fixed zero div error
# made data more precise.  


##############################################     to do list
############## need to add chunks on, currently only adding terms to chunks that allready exist
##############   is it currently possible to do     t = (2*(s-u))/a    ..... i think i need to nest the chunk in further chunks again   , or maybe just more ops 
##############   somehow more terms are added than removed over time .... or more terms = more correct.        check thst the later is the case, else fix.


##########################################################################
######  old to do list below , some things might still need doing double check 
############## wtf only doing first bracket?    # search in comment     ## double check 
############## overflow evveror
...
#
###############################################
#####    ensure every single index is more accurate each time, then for the ones that were all correct for bestr rmse     ####### right now half of data is like 10 -6 off 


max_brackets_per_neuron = 2
max_bracket_terms = 2
base_layer_pop = 50
   ...
   #     print("bart to getr diffs")
    ...
    #    print(" GOT DIFS ")
        ...
        for d in range(len(difs)):
            if error == False:
                if type(d) == type("lol"):
                    if "zero div error" in d:
                        print("     zerror div error  "+str(d))
                        error = True;
                try:
                    a = 1/ difs[d]
        ...
        if diffs_that_counter > 0 and error == False:
            try:
     ...
     #       time.sleep(19)
            ...
            mse = 999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999
...
fail_counter = 0
allfinaldf = pd.DataFrame({'counter':[],'mse':[],'full operation':[], 'full operation values':[],  })
     ...
     #               print(str(new_mse)+ " = bad new mse   ,   which aint as good as = "+ str(best_mse))
      ...
      #          print(" ")
            ...
            if not reset:
                ...
                allfinaldf = allfinaldf.append(newrow)
              #  if counter > 15:
             #       finaldf.to_csv("good sequential attempts ="+str(counter)+"     total rounds = "+str(max_counter)+".csv")
                try:
                    allfinaldf.to_csv("all.csv")
                except:
                    allfinaldf.to_csv("all_spare.csv")
                fail_counter = 0
