...
#####################################
    ...
    def describe_neurons_evolution(self):
        ...
        while the_neuron_were_currently_using.parent_neuron != 0:
            print(" ")
            the_neuron_were_currently_using.describe_neuron()
    ...
    def perform_calculation_at_data_index(self , index):
        try:
            ...
            for m in range(len(self.outer_bracket_chunks.mid_brackets)):
                ...
                for i in range(len(self.outer_bracket_chunks.mid_brackets[m].inner_brackets)):
                    this_inner_total = 0
                    for inner_index in range(len(self.outer_bracket_chunks.mid_brackets[m].inner_brackets[i].polynomials)-1):
...
#    try:
    ...
    while 1==1:
        layer_num_counter += 1
        if layer_num_counter%100 == 0:
            if abs(last_100_rmse - this_best_neuron.get_rmse()) < 1:
   ...
   #     for n in layer2.neurons:
    ...
    #    this_best_neuron.describe_neurons_evolution();
        ...
        if type(this_best_neuron) == type("lol"):
            ...
            print(str(this_best_neuron.get_rmse()) + " =   rmse             ,    were good. moving on to next layer ")
